...
**Sobre a API oficial do ML** (correção de 12/07, após verificação): existe `netlify/functions/search.js` usando a API oficial (que devolveria link direto + foto + preço em tempo real), mas ela está morta por DOIS motivos: (a) as credenciais configuradas no Netlify estão inválidas (`invalid_client`, testado em produção), e (b) mais importante, **o Mercado Livre vem bloqueando o endpoint de busca `/sites/MLB/search` para desenvolvedores desde 2025** (erro 403 mesmo com token válido — há dezenas de relatos públicos e a Priscila estava certa sobre isso). Ou seja: religar a busca oficial NÃO é caminho viável hoje. Os caminhos reais para o ML são: consertar o retorno vazio do Apify (curto prazo) e, na Onda 2, testar se o endpoint de **item individual** (`/items/{id}`) continua acessível — se sim, IDs curados no catálogo dão preço em tempo real + link direto sem depender da busca bloqueada.
...
4. **Mercado Livre**: descobrir por que o Apify retorna vazio (créditos? actor mudou o formato? testar no console da Apify) e consertar — a busca oficial do ML está bloqueada para desenvolvedores (403 desde 2025), então o scraper É o caminho de curto prazo. Enquanto isso, nunca cachear vazio e mostrar o card sem a linha ML em vez de silenciar o problema.
...
4. **Mercado Livre**: a busca oficial está bloqueada (a Priscila estava certa). Decidir: investir em consertar/estabilizar o Apify agora, e na Onda 2 testar o endpoint de item individual (`/items/{id}`) com IDs curados — se funcionar, é o melhor dos dois mundos (preço em tempo real + link direto, sem depender da busca bloqueada).
