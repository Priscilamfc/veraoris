...
async function fetchFeed(query) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log('MAHOGANY resposta não-OK, status:', res.status);
    }
  } catch (err) {
    console.log('MAHOGANY fetch falhou:', err.message);
  } finally {
    clearTimeout(timer);
  }

  const products = normalizeItems(raw);
  console.log('MAHOGANY produtos válidos:', products.length, '/', raw.length);
...
export default async (request) => {
  ...
  try {
    ...
    console.log('MAHOGANY erro:', error.message);
    return new Response(JSON.stringify({ results: [], error: error.message }), { status: 200, headers });
  }
};

export const config = { path: '/.netlify/functions/mahogany-search' };
