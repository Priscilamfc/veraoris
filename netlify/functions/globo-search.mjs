// Drogaria Globo — chamada DIRETA na API pública do VTEX, mesmo padrão de
// Época/WePink/Americanas/Mahogany/Lojas Pompéia/Extrafarma/Venâncio/Drogal/Pague
// Menos/Catarinense/São João/Rosário/Payot/Preço Popular. Rede de farmácia (Grupo Jorge
// Batista) — achada e testada ao vivo em 05/08/2026, mesmo pedido da Priscila (aumentar
// quantidade de lojas/produtos, mesmo sem comissão de afiliado). Traz marca de farmácia real
// (Bepantol, Fisiogel testados ao vivo).
// Farmácia (vende de tudo) — filtra pelo campo `categories`. Espalha beleza em 2 raízes:
// "/Dermocosméticos/" (corpo) e "/Higiene & Beleza/" (cabelo, pele, maquiagem — testado com
// "batom", tudo cai aqui). Achada contaminação testando "hidratante": gel vaginal aparece sob
// "/Saúde/" — não entra na lista.
const CACHE_TTL_MS = 30 * 60 * 1000;
const FETCH_TIMEOUT_MS = 8000;
const API_URL = 'https://www.drogariaglobo.com.br/api/catalog_system/pub/products/search';
const BEAUTY_CATEGORY_PREFIXES = ['/dermocosméticos', '/higiene & beleza'];
...
function guessSubcat(categories) {
  ...
  // "Leite de Colônia" (loção corporal, a própria Globo classifica como "/Higiene &
  // Beleza/Cuidados com a pele/") tem "colônia" no título mas é skincare de verdade --
  // achado ao vivo 06/08/2026, mesmo padrão aplicado a todas as functions de farmácia.
  ...
  return (raw || [])
    ...
    .map((p) => {
      ...
      return {
        title: p.productName,
        price,
        store: 'Drogaria Globo',
        link: 'https://www.drogariaglobo.com.br/' + encodeURIComponent(p.linkText) + '/p',
        image: image || null,
        brand,
...
async function fetchFeed(query) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log('GLOBO resposta não-OK, status:', res.status);
    }
  } catch (err) {
    console.log('GLOBO fetch falhou:', err.message);
  } finally {
    clearTimeout(timer);
  }

  const products = normalizeItems(raw);
  console.log('GLOBO produtos válidos:', products.length, '/', raw.length);
...
export default async (request) => {
  ...
  try {
    ...
    console.log('GLOBO erro:', error.message);
    return new Response(JSON.stringify({ results: [], error: error.message }), { status: 200, headers });
  }
};

export const config = { path: '/.netlify/functions/globo-search' };
