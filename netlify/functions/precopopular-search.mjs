// Farmácia Preço Popular — chamada DIRETA na API pública do VTEX, mesmo padrão de
// Época/WePink/Americanas/Mahogany/Lojas Pompéia/Extrafarma/Venâncio/Drogal/Pague
// Menos/Catarinense/São João/Rosário/Payot. Rede de farmácia (Grupo Clamed) — achada e
// testada ao vivo em 05/08/2026, mesmo pedido da Priscila (aumentar quantidade de lojas/
// produtos, mesmo sem comissão de afiliado). Traz marca de farmácia real (Neutrogena, Garnier
// testados ao vivo).
// Farmácia (vende de tudo) — filtra pelo campo `categories`. Espalha beleza em 2 raízes:
// "/Beleza e Proteção/" (cabelo, maquiagem, lábios) e "/Dermocosméticos/" (rosto). Achada
// contaminação testando "hidratante": hidratante infantil (Johnson Baby) aparece sob
// "/Mamãe e Bebê/" — não entra na lista, só as 2 raízes de beleza de verdade.
const CACHE_TTL_MS = 30 * 60 * 1000;
const FETCH_TIMEOUT_MS = 8000;
const API_URL = 'https://www.precopopular.com.br/api/catalog_system/pub/products/search';
const BEAUTY_CATEGORY_PREFIXES = ['/beleza e proteção', '/dermocosméticos'];
  ...
  return (raw || [])
    ...
    .map((p) => {
      ...
      return {
        title: p.productName,
        price,
        store: 'Farmácia Preço Popular',
        link: 'https://www.precopopular.com.br/' + encodeURIComponent(p.linkText) + '/p',
        image: image || null,
        brand
      };
    })
...
async function fetchFeed(query) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log('PRECOPOPULAR resposta não-OK, status:', res.status);
    }
  } catch (err) {
    console.log('PRECOPOPULAR fetch falhou:', err.message);
  } finally {
    clearTimeout(timer);
  }

  const products = normalizeItems(raw);
  console.log('PRECOPOPULAR produtos válidos:', products.length, '/', raw.length);
...
export default async (request) => {
  ...
  try {
    ...
    console.log('PRECOPOPULAR erro:', error.message);
    return new Response(JSON.stringify({ results: [], error: error.message }), { status: 200, headers });
  }
};

export const config = { path: '/.netlify/functions/precopopular-search' };
